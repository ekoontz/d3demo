...
var svg = d3.select("#chart").append("svg")
  ...
  .append("g")
    .attr("transform", "translate(" + x_axis(1) + "," + (height - 20) + ")scale(-1,-1)");
...
var title = svg.append("text")
    .attr("class", "title")
    .attr("dy", ".71em")
    .attr("transform", "translate(" + x_axis(1) + "," + y_axis(1) + ")scale(-1,-1)")
    .text(2000);
...
d3.csv("./population.csv", function(data) {
  ...
  x_axis.domain([0, max_age + 5]);
  y_axis.domain([0, d3.max(data, function(d) { return d.people; })]);
  ...
  rules = rules.selectAll(".rule")
    ...
    .enter().append("g")
      .attr("class", "rule")
      .attr("transform", function(d) { return "translate(0," + y_axis(d) + ")"; });
  ...
  var years = body.selectAll("g")
    ...
    .enter().append("g")
      .attr("transform", function(d) { return "translate(" + x_axis(max_year - d) + ",0)"; });

  years.selectAll("rect")
      .data(d3.range(2))
    .enter().append("rect")
      .attr("x", 1)
      .attr("width", x_axis(5) - 2)
      .attr("height", 1e-6);
  ...
  svg.append("g").selectAll("text")
    ...
    .enter().append("text")
      .attr("text-anchor", "middle")
      .attr("transform", function(d) { return "translate(" + (x_axis(d) + x_axis(5) / 2) + ",-4)scale(-1,-1)"; })
      .attr("dy", ".71em")
      .text(String);
